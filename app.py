...
st.title("🍽 식단 기록 + 딥러닝 기반 맞춤 메뉴 추천 웹")
...
# UI: 탭 구성
# =========================
tab1, tab2 = st.tabs(["🥗 식단 기록하기", "🤖 딥러닝 메뉴 추천"])
...
# -------------------------
with tab2:
    st.subheader("2️⃣ 딥러닝 기반 맞춤 메뉴 추천 받기")

    if len(df) < 20:
        st.warning("딥러닝 모델을 학습하기에는 데이터가 너무 적습니다. 최소 20개 이상 기록해 주세요.")
    else:
        ...
        if st.button("딥러닝으로 메뉴 추천 받기 👇"):
            with st.spinner("모델 학습/로딩 중입니다..."):
...
st.markdown("---")
st.caption("개인 식단 데이터를 활용한 딥러닝 기반 맞춤 메뉴 추천 앱")
